...
ndim = 3;         %Removed Tb from state and meas vectors.
...
Ccp1 = -2285;
Ccp2 = -0.73;
Rp = 1e5;  %From Ed Hogge BHM Checkcase
Jt = 800;
hcp = 19;
hcs = 1;
...
x(:,1) = [qmax qmax qmax]'; %Initial state condition
SOC_array = ones(1,N);
SOC_array(1) = 1;
...
%qb_display = state_ukf(1,2)

%t_to_volt(k) = t_predict;
%P = P;        %Reminder of P update
x(:,k) = state_ukf(:,2);
x_est(:,k) = est_ukf(:,2);
SOC_array(k)= SOC;
end
plot_results(SOC_array,i_frombattery_model,N,x_total,z,x_est,dt,t_EOD,SOC_EOD, counter_array)
...
function [] = plot_results(SOC_array,i_frombattery_model,N,x_total,z,x_est,dt,t_EOD,SOC_EOD, counter_array)
%% Plot Results %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%figure  %Makes a new instance of charts
subplot
k = 1:N;
subplot(2,2,1)
plot(k*dt,i_frombattery_model,'r',k*dt,SOC_array*100,'bo',k*dt,z,'g',k*dt,SOC_EOD*100*ones(1,N),'c--')
legend('battery current','B3 SOC','Vp','SOC_{EOD}')
ylim([0 100]);
