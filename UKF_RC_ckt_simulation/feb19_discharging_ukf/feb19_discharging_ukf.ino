  ...
  const float tau = 3300*pow(10,-6)*180;
  const float xa0 = 5;
 // float xa = xa0;
  float xa = 1;
  float x = 5;
...
void setup(){
 ...
 // Serial.println("LABEL,Timer,truth,estimate");
  Serial.println("LABEL,Timer,t_remain,measurement");
  pinMode(A0, OUTPUT);
...
//  k++;
  ...
  */
  ...
  if (t_remain > 0 && -tau*log(2.0/x_est)<0)
  {
    t_remain = -millis()/1000;
  }
  else
  {
    t_remain = -tau*log(2.0/x_est);
  }
 
  Serial.print("DATA,TIMER,"); Serial.print(t_remain);Serial.print(","); Serial.println(z);
  row++;
  delay(10);
}
